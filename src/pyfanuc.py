...
if __name__ == '__main__':
	conn=pyfanuc('10.10.0.95')
	if conn.connect():
		print("connected")
		axis = conn.readaxes(conn.ABS)['ABS']
		x_axis = axis[0]
		z_axis = axis[1]
		program = conn.readexecprog()
...
#			print(n['name']+" ("+time.strftime("%c",n['datetime'])+')' if n['type']=='F' else '<'+n['name']+'>')
		...
		try:
			while True:
				compaxis = conn.readaxes(conn.ABS)['ABS']
				comprogram = conn.readexecprog()
				# print(comprogram)
				if compaxis[0] != x_axis or compaxis[1] != z_axis :
					x_axis = compaxis[0]
					z_axis = compaxis[1]
					print(x_axis,z_axis,datetime.datetime.now())
				if conn.readexecprog() != program:
					program = conn.readexecprog()
					print(program,datetime.datetime.now())
